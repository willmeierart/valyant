...
export const getVPDims = () => async dispatch => {
  const getDims = () => {
    if (typeof window !== 'undefined') {
      ...
      const scale = false
      const isMobile = typeof window.orientation !== 'undefined' && Math.abs(window.orientation) === 90 // add this false bool for testing innerWidth
      const WH = {
...
export const setSideTagCurrentHeight = h => async dispatch => {
  dispatch({
    type: SET_SIDE_TAG_FALLBACK_HEIGHT,
    payload: h
  })
}

export const setSideTagFallbackHeight = h => async dispatch => {
  dispatch({
    type: SET_SIDE_TAG_CURRENT_HEIGHT,
    payload: h
  })
}
